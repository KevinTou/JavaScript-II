...
const totalDonations = (total, currentDonation) => total + currentDonation;
ticketPriceTotal = runners
  .map(runner => {
    return runner.donation;
  })
  .reduce(totalDonations);
...
// Problem 1
// Potentially use for future events
const getEmails = runners.map(runner => {
  return runner.email;
});
...
const giveaway = [];
runners.forEach(runner => {
  giveaway.push(runner.company_name);
});
const grandPrizeWinner = giveaway.splice(Math.random() * giveaway.length, 1);
console.log(grandPrizeWinner);

// Problem 3
// Tally up shirt sizes to avoid running out of shirts
let shirtSizes = [];
shirtSizes = runners.map(runner => {
